...
namespace RegionKit
{
	...
	public partial class RegionKitMod : BaseUnityPlugin
	{
		...
		public void OnEnable()
		{
			...
			try
			{
				if (RKEnv.RulesDet is not null && RKEnv.RulesDet.TryGetValue("RKLogOutput", out IEnumerable<string>? prm))
				{
					woodpath = prm?.FirstOrDefault();
				}
				bool lsf = RKEnv.Rules?.Contains("RKLogFile") ?? false;// ?? false;
				...
				foreach (Partiality.Modloader.PartialityMod? mod in Partiality.PartialityManager.Instance.modManager.loadedMods)
				{
		...
		public void OnDisable()
		{
			...
			foreach (Type? t in casm.GetTypes()) if (t != typeof(PWood)) t.CleanUpStatic();
				else
				{
