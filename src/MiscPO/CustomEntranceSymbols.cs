...
namespace RegionKit.MiscPO
{
		...
		public static void ApplyHooks()
		{
			On.ShortcutGraphics.Draw += (orig, self, timeStacker, camPos) =>
			{
				...
				for (int l = 0; l < self.room.shortcuts.Length; l++)
				{
					...
					{
						PlacedObject pObj = self.room.roomSettings.placedObjects[m];
			...
			On.DevInterface.ObjectsPage.CreateObjRep += (orig, self, tp, pObj) =>
			{
				if (tp == EnumExt_CES.CustomEntranceSymbol)
				{
					...
					CESRepresentation pObjRep = new(self.owner, tp.ToString() + "_Rep", self, pObj);
	...
	public class CESData : PlacedObject.Data
	{
		...
		public CESData(PlacedObject owner) : base(owner)
		{
		...
		{
			return $"{panelPos.x}~{panelPos.y}~{imageName}~{rotation}";
		}
	}
	...
	public class CESRepresentation : TileObjectRepresentation
	{
		internal class CESControlPanel : Panel, IDevUISignals
		{
			...
			public void Signal(DevUISignalType type, DevUINode sender, string message)
			{
				...
				{
					int num = int.Parse(sender.IDstring.Substring(19));
					float nP = (rep.files.Length / 10f) - 1f;
					if (num < nP)
					{
						num++;
				...
				{
					int num = int.Parse(sender.IDstring.Substring(23));
					if (num > 0)
					{
						num--;
						SelectSpritePanel.OrganizeSprites(num, spriteSelectPanel);
		...
		public CESRepresentation(DevUI owner, string IDstring, DevUINode parentNode, PlacedObject pObj) : base(owner, IDstring, parentNode, pObj, pObj.type.ToString())
		{
			...
			int i = 0;
			foreach (System.Collections.Generic.KeyValuePair<string, FAtlasElement> item in Futile.atlasManager._allElementsByName)
			{
