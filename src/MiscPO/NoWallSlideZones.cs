...
namespace RegionKit.MiscPO
{
		...
		public static void Apply()
		{
			...
			On.DevInterface.ObjectsPage.CreateObjRep += (orig, self, tp, pObj) =>
			{
				if (tp == EnumExt_NoWallSlideZones.NoWallSlideZone)
				{
					...
					FloatRectRepresentation pObjRep = new(self.owner, $"{tp}_Rep", self, pObj, tp.ToString());
	...
	public class NoWallSlideZone : UpdatableAndDeletable
	{
		...
		{
			base.Update(eu);
			FloatRect r = (pObj.data as FloatRectData).Rect;
	...
	public class FloatRectData : PlacedObject.Data
	{
		...
		public FloatRectData(PlacedObject owner) : base(owner)
		{
			handlePos = new(80f, 80f);
		}
		...
		{
			return $"{handlePos.x}~{handlePos.y}";
		}
	}
	...
	{
		public static float Height(this FloatRect self)
		{
			return Math.Abs(self.top - self.bottom);
		}

		public static float Width(this FloatRect self)
		{
			return Math.Abs(self.right - self.left);
		}

		public static float Area(this FloatRect self)
		{
			return self.Height() * self.Width();
		}

		public static bool InsideRect(Vector2 vec, FloatRect rect)
		{
			return vec.x >= rect.left && vec.x <= rect.right && vec.y >= rect.bottom && vec.y <= rect.top;
		}

		public static bool InsideRect(float x, float y, FloatRect rect)
		{
			return x >= rect.left && x <= rect.right && y >= rect.bottom && y <= rect.top;
		}

		public static bool EqualsFloatRect(this FloatRect self, FloatRect other)
		{
			return self.left == other.left && self.right == other.right && self.bottom == other.bottom && self.top == other.top;
		}
