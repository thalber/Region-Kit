...
namespace RegionKit.MiscPO
{
	public class LittlePlanet : CosmeticSprite
	{
		public static void ApplyHooks()
		{
			...
			On.DevInterface.ObjectsPage.CreateObjRep += (orig, self, tp, pObj) =>
			{
				if (tp == EnumExt_LittlePlanet.LittlePlanet)
				{
					...
					LittlePlanetRepresentation pObjRep = new(self.owner, "LittlePlanet_Rep", self, pObj, tp.ToString());
		...
		public class LittlePlanetData : PlacedObject.ResizableObjectData
		{
			...
			{
				return $"{base.ToString()}~Color(~{red}~{green}~{blue}~)~Alpha:~{alpha}~{panelPos.x}~{panelPos.y}~Speed:~{speed}";
			}
		}
		...
		public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
		{
			...
			for (int i = 1; i < sLeaser.sprites.Length - 1; i++)
		...
		{
			Vector2 sPos = Vector2.Lerp(lastPos, pos, timeStacker);
			float[] sRot = new[]
			{
	...
	public class LittlePlanetRepresentation : ResizeableObjectRepresentation
	{
		public class LittlePlanetControlPanel : Panel, IDevUISignals
		{
			public class ControlSlider : Slider
			{
				...
				{
					Data = (parentNode.parentNode as LittlePlanetRepresentation).pObj.data as LittlePlanet.LittlePlanetData;
				}
	...
	public static class EmbeddedResourceLoader
	{
		public static void LoadEmbeddedResource(string name)
		{
			...
			string resourceName = thisAssembly.GetManifestResourceNames().First(r => r.Contains(name));
			Stream resource = thisAssembly.GetManifestResourceStream(resourceName);
