...
namespace RegionKit.AridBarrens
{
	public class SandStorm : BackgroundScene, INotifyWhenRoomIsReady
	{
		...
		private RainCycle cycle => this.room.game.world.rainCycle;

		public float Intensity => this.effect.amount * Mathf.Pow(Mathf.InverseLerp((float)(this.cycle.cycleLength - 400), (float)(this.cycle.cycleLength + 2400), (float)this.cycle.timer), 2.2f);
		...
		public override void Update(bool eu)
		{
			...
			if (this.soundLoop == null)
			{
				...
				{
					sound = SoundID.Void_Sea_Worm_Swimby_Woosh_LOOP,
					Volume = 0f
				};
			}
			else
			...
			if (this.soundLoop2 == null)
			{
				...
				{
					sound = SoundID.Gate_Electric_Steam_LOOP,
					Volume = 0f
				};
			}
			else
		...
		private void AddSpark()
		{
			...
			if (!this.room.GetTile(pos).Solid)
			{
				...
				SandPart particle = new(vector);
				this.room.AddObject(particle);
				this.particles.Add(particle);
			}
		}

		public float InsidePushAround => this.effect.amount * Intensity;
		...
		public class SandPuff : CosmeticSprite
		{
			...
			public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
			{
				...
				{
					shader = rCam.game.rainWorld.Shaders["Spores"]
				};
		...
		public class Fog : BackgroundScene.FullScreenSingleColor
		{
			...
			private float Intensity => (base.scene as SandStorm).Intensity;


			private SandStorm SandStormScene => this.scene as SandStorm;
			...
			public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
			{
				...
				{
					scaleX = (rCam.game.rainWorld.screenSize.x + 20f) / 1f,
					scaleY = (rCam.game.rainWorld.screenSize.y + 20f) / 1f,
					x = rCam.game.rainWorld.screenSize.x / 2f,
					y = rCam.game.rainWorld.screenSize.y / 2f,
