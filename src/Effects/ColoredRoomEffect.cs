...
namespace RegionKit.Effects
{
    public static class ColoredRoomEffect /// By M4rbleL1ne/LB Gamer
	{
        ...
        internal static void Apply()
        {
            ...
            {
                string reg = GRKString(self);
                string res = orig(self);
                if (colorSettings.TryGet(self, out ColorSettings? cs) && cs.colored)
            ...
            On.DevInterface.EffectPanel.ctor += (orig, self, owner, parentNode, pos, effect) =>
            {
                ...
                if (colorSettings.TryGet(effect, out ColorSettings? cs) && cs.colored)
                {
                    self.size.y += 60f;
                    int indSn = -1;
                    int ftSn = -1;
            ...
            {
                orig(self);
                if (colorSettings.TryGet(self.effect, out ColorSettings? cs) && cs.colored)
                {
                    float num = 0f;
            ...
            {
                if (!self.effect.inherited && colorSettings.TryGet(self.effect, out ColorSettings? cs) && cs.colored)
                {
                    switch (self.IDstring)
                    {
                    case "Amount_Slider":
                        self.effect.amount = nubPos;
                        RoomSettings.RoomEffect.Type type = self.effect.type;
        ...
        public static bool IsEffectInRoom(this RoomSettings self, RoomSettings.RoomEffect.Type type)
        {
            return self.GetEffect(type) != null;
        }
