...
namespace RegionKit.Effects
{
	...
	public class FogOfWarController : CosmeticSprite
	{
		...
		public static RoomSettings.RoomEffect.Type GetEffectType(FogType fogType)
		{
			...
			{
				FogType.Darkened => EnumExt_Effects.FogOfWarDarkened,
				FogType.Solid => EnumExt_Effects.FogOfWarSolid,
		...
		public FogType Type
		{
			get
			{
				...
				{
					FogType type = GetFogType(effect.type);
		...
		public FogOfWarController(Room room)
		{
			...
			if (fovShader == null)
			{
				Material mat = new(Shaders.FogOfWar);
		...
		public List<Vector2> corners = new();
		...
		private bool HasEdge(int x, int y, Direction dir)
		{
			...
			if (terrain is Room.Tile.TerrainType.Air or
				Room.Tile.TerrainType.ShortcutEntrance or
				Room.Tile.TerrainType.Floor) return false;
			return dir switch
			{
				Direction.Up => slope is Room.SlopeDirection.DownRight or Room.SlopeDirection.DownLeft,
				Direction.Right => slope is Room.SlopeDirection.UpLeft or Room.SlopeDirection.DownLeft,
				Direction.Down => slope is Room.SlopeDirection.UpRight or Room.SlopeDirection.UpLeft,
				Direction.Left => slope is Room.SlopeDirection.DownRight or Room.SlopeDirection.UpRight,
				_ => false,
			};
		}
		...
		public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
		{
			...
			TriangleMesh colorBlocker = new("Futile_White", tris, false, true)
			{
				shader = shader
			};
			...
			TriangleMesh scPeek = new("Futile_White", tris, true, true);
		...
		{
			Color col = Color.Lerp(palette.blackColor, palette.fogColor, Lightness);
			col.a = Amount;
