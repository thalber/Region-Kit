	...
	public class GlowingSwimmersCI
	{
		...
		public static void Apply()
		{
			...
			On.InsectCoordinator.CreateInsect += (orig, self, type, pos, swarm) =>
			{
				...
				{
					GlowingSwimmer cosmeticInsect = new(self.room, pos);
	...
	public class GlowingSwimmer : CosmeticInsect
	{
		...
		public override void Update(bool eu)
		{
			...
			for (int i = 0; i < segments.GetLength(0); i++)
			{
				...
				if (i is 0)
				{
					Vector2 vector = DirVec(segments[i, 0], pos);
				...
				{
					Vector2 vector2 = DirVec(segments[i, 0], segments[i - 1, 0]);
		...
		public override void Act()
		{
			...
			float num = Mathf.Pow(creatureAvoider.FleeSpeed, .3f);
			if (num > stressed)
		...
		public virtual int LegSprite(int segment, int leg)
		{
			return (segment * 2) + leg;
		}

		public virtual int SegmentSprite(int segment, int part)
		{
		...
		public override void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
		{
			...
			for (int i = 0; i < segments.GetLength(0) + 1; i++)
			{
				float t = .5f + (.5f * Mathf.Sin((Mathf.Lerp(lastBreath, breath, timeStacker) + (.1f * i)) * 7f * (float)Math.PI));
				Vector2 p = (i != 0) ? segments[i - 1, 0] : Vector2.Lerp(lastPos, pos, timeStacker);
				Vector2 v = i switch
				{
				...
				p.y -= 5f * num;
				float num3 = LerpMap(i, 0f, segments.GetLength(0), 1f, .5f, 1.2f) * (1f - num);
				for (int j = 0; j < 2; j++)
				{
					FSprite seg = sLeaser.sprites[SegmentSprite(i, j)];
					seg.x = p.x - camPos.x;
					seg.y = p.y - camPos.y;
					seg.rotation = VecToDeg(v);
					seg.scaleX = 4f * num3 * (1f - num) / 15f;
					seg.scaleY = 6.5f * num3 * (1f - num) / 15f;
					FSprite leg = sLeaser.sprites[LegSprite(i, j)];
