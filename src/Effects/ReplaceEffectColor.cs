...
namespace RegionKit.Effects
{
	public class ReplaceEffectColor : UpdatableAndDeletable /// By M4rbleL1ne/LB Gamer
	{
		...
		public ReplaceEffectColor(Room room)
		{
			this.room = room;
		}
		...
		public override void Update(bool eu)
		{
			...
			if (room?.game is not null)
			{
				foreach (RoomCamera? cam in room.game.cameras)
				{
					...
					{
						float a = cam.room.roomSettings.GetEffectAmount(ReplaceEffectColorA);
						float b = cam.room.roomSettings.GetEffectAmount(ReplaceEffectColorB);
						float clrar = cam.room.roomSettings.GetColoredEffectRed(ReplaceEffectColorA);
						float clrbr = cam.room.roomSettings.GetColoredEffectRed(ReplaceEffectColorB);
						float clrag = cam.room.roomSettings.GetColoredEffectGreen(ReplaceEffectColorA);
						float clrbg = cam.room.roomSettings.GetColoredEffectGreen(ReplaceEffectColorB);
						float clrab = cam.room.roomSettings.GetColoredEffectBlue(ReplaceEffectColorA);
						float clrbb = cam.room.roomSettings.GetColoredEffectBlue(ReplaceEffectColorB);
