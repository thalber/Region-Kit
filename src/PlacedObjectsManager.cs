...
#pragma warning restore IDE0051 // Remove unused private members
		...
		#region INTERNALS
		private static readonly List<ManagedObjectType> managedObjectTypes = new();
		/// <summary>
		...
		{
			foreach (ManagedObjectType manager in managedObjectTypes)
			{
		...
		{
			ManagedObjectType emptyObjectType = new(name, null, dataType, reprType);
		...
		public class ManagedObjectType
		{
			...
			public virtual UpdatableAndDeletable MakeObject(PlacedObject placedObject, Room room)
			{
				...
				if (singleInstance) // Only one per room
				{
					...
					foreach (UpdatableAndDeletable? item in room.updateList)
					{
		...
		public abstract class ManagedField : Attribute
		{
			...
			/// </summary>
			public virtual bool NeedsControlPanel => this is ManagedFieldWithPanel;
		...
		public abstract class ManagedFieldWithPanel : ManagedField
		{
			...
			public virtual PositionedDevUINode MakeControlPanelNode(ManagedData managedData, ManagedControlPanel panel, float sizeOfDisplayname)
			{
				...
				{
					ControlType.slider => new ManagedSlider(this, managedData, panel, sizeOfDisplayname),
					ControlType.arrows => new ManagedArrowSelector(this, managedData, panel, sizeOfDisplayname),
					ControlType.button => new ManagedButton(this, managedData, panel, sizeOfDisplayname),
					ControlType.text => new ManagedStringControl(this, managedData, panel, sizeOfDisplayname),
					_ => null,
				};
			}
		...
		public class ManagedData : PlacedObject.Data
		{
			...
			{
				paramFields ??= new ManagedField[0];
				...
				List<ManagedField> attrFields = new();
				...
				foreach (ManagedField field in this.fields)
				{
				...
				foreach (FieldInfo fieldInfo in this.GetType().GetFields())
				{
					...
					foreach (object attr in customAttributes) // There should be only one anyways ??? As long as they have different keys everything will be fiiiiine
					{
		...
		public class ManagedRepresentation : PlacedObjectRepresentation
		{
			...
			protected virtual void MakeControls()
			{
				...
				{
					ManagedControlPanel panel = new(this.owner, "ManagedControlPanel", this, data.panelPos, Vector2.zero, pObj.type.ToString());
					this.panel = panel;
					this.subNodes.Add(panel);
					Vector2 uiSize = new(0f, 0f);
					Vector2 uiPos = new(3f, 3f);
		...
		public class EnumField : ManagedFieldWithPanel, IIterablePanelField, IInterpolablePanelField
		{
			...
			protected virtual Enum[] PossibleValues // We defer this listing so enumextend can do its magic.
			{
				get
				{
					_possibleValues ??= Enum.GetValues(type).Cast<Enum>().ToArray();
		...
		public class StringField : ManagedFieldWithPanel
		{
			...
			protected readonly static Dictionary<string, string> replacements = new()
		{
			{ ": ","%1" },
			{ ", ","%2" },
			{ "><","%3" },
		...
		public class ColorField : ManagedFieldWithPanel, IInterpolablePanelField
		{
			...
			{
				return this.control switch
				{
					ControlType.slider => HUD.DialogBox.meanCharWidth * (displayName.Length + 6),
					_ => base.SizeOfDisplayname(),
				};
			}
		...
		public class DrivenVector2Field : Vector2Field
		{
			...
			public override DevUINode MakeAditionalNodes(ManagedData managedData, ManagedRepresentation managedRepresentation)
			{
				return drivenControlType switch
				{
					DrivenControlType.relativeLine => new DrivenVectorControl(this, managedData, managedRepresentation.managedNodes[keyOfOther] as PositionedDevUINode, drivenControlType, label),
					DrivenControlType.perpendicularLine or DrivenControlType.perpendicularOval or DrivenControlType.rectangle => new DrivenVectorControl(this, managedData, managedRepresentation, drivenControlType, label),
					_ => null,
				};
			}
		...
		public class ManagedIntHandle : Handle // All-in-one super handle 2
		{
			...
			public override void Move(Vector2 newPos)
			{
				...
				RWCustom.IntVector2 ownIntPos = new(Mathf.FloorToInt(roompos.x / 20f), Mathf.FloorToInt(roompos.y / 20f));
				RWCustom.IntVector2 parentIntPos = new(Mathf.FloorToInt(parentPos.x / 20f), Mathf.FloorToInt(parentPos.y / 20f));
				// relativize again
		...
		public class ManagedStringControl : PositionedDevUINode
		{
			...
			{
				get => _text;// subNodes[1].fLabels[0].text;
				set
				{
					_text = value;
