...
namespace RegionKit.Particles
{
	...
	public class RoomParticleSystem : UpdatableAndDeletable
	{
		...
		public RoomParticleSystem(PlacedObject owner, Room room, Vector2 oPos, ParticleSystemData npsd, params ParticleCreate[] births)
		{
			...
			if (births != null) foreach (ParticleCreate d in births) BirthEvent += d;
		...
		{
			float TotalModCost = 1f;
			GenericParticle dummy = new(default, default);
			foreach (ParticleBehaviourProvider mod in Modifiers)
			{
				TotalModCost += Max(0f, mod.GetNewForParticle(dummy)?.ComputationalCost ?? 0f);
			}
			Delegate[] pBirths = BirthEvent?.GetInvocationList() ?? new Delegate[0];
		...
		public override void Update(bool eu)
		{
			if (room.waitToEnterAfterFullyLoaded > 0 && DelayRequestedByMe > 0)
			{
				...
				{
					GenericParticle lccp = ProgressCreationCycle();
		...
		protected virtual GenericParticle ProgressCreationCycle()
		{
			...
			if (cooldown <= 0)
			{
				Delegate[]? PossibleBirths = BirthEvent?.GetInvocationList();
				if (PossibleBirths != null && PossibleBirths.Length > 0)
				{
					...
					p.pos = PickSpawnPos();
					foreach (ParticleBehaviourProvider provider in Modifiers)
					{
						PBehaviourModule newmodule = provider.GetNewForParticle(p);
		...
		{
			List<IntVector2> tiles = PSD.ReturnSuitableTiles(room);
			if (tiles.Count == 0) { tiles.Add((MyPos / 20).ToIV2()); }
			IntVector2 tile = tiles[UnityEngine.Random.Range(0, tiles.Count)];
			return new Vector2()
			{
