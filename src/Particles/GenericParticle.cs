...
namespace RegionKit.Particles
{
	...
	public class GenericParticle : CosmeticSprite
	{
		...
		public override void Update(bool eu)
		{
			...
			float cpw = CurrentPower;
			float crd = cRad(cpw);
			float cLInt = (visuals.lInt > 0f) ? Lerp(0f, visuals.lInt, cpw) : 0f;
			if (!SetUpRan)
			{
				foreach (PBehaviourModule m in Modules) m.Enable();
				OnCreate?.Invoke();
		...
		{
			OnDestroy?.Invoke();
			foreach (PBehaviourModule m in Modules) m.Disable();
			base.Destroy();
		}
		...
		protected virtual float cRad(float power)
		{
			return Lerp(visuals.lRadMin, visuals.lRadMax, power);
		}

		/// <summary>
		/// 0 to 1; represents how thick/transparent a particle is at the moment
		/// </summary>
		public virtual float CurrentPower => phase switch
		{
		...
		public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
		{
			...
			room.game.rainWorld.Shaders.TryGetValue("Basic", out FShader? sh);
		...
		{
			Vector2 cpos = Vector2.Lerp(lastPos, pos, timeStacker);
