...
namespace RegionKit.Particles
{
	...
	public class OffscreenSpawnerData : ParticleSystemData
	{
		...
		{
			List<IntVector2> res = new();
			IntRect rb = new(0 - margin, 0 - margin, rm.Width + margin, rm.Height + margin);
			Vector2 dropVector = GetValue<Vector2>("sdBase");
	...
	public class WholeScreenSpawnerData : ParticleSystemData
	{
		...
		protected override List<IntVector2> GetSuitableTiles(Room rm)
		{
			List<IntVector2> r = ConstructIR(new IntVector2(-Margin, -Margin), new IntVector2(rm.TileWidth + Margin, rm.TileHeight + Margin)).ReturnTiles();
			if (AirOnly)
			{
