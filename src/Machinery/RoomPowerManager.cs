...
namespace RegionKit.Machinery
{
	...
	public class RoomPowerManager : UpdatableAndDeletable
	{
		...
		internal PowerManagerData pmData { get { _pmd ??= PO?.data as PowerManagerData ?? new PowerManagerData(null); return _pmd; } }
		...
		{
			float res = pmData.basePowerLevel;
			res += GetGlobalPower();
			foreach (IRoomPowerModifier unit in subs) if (unit.Enabled) res += unit.BonusForPoint(point);
			res = Clamp01(res);
			return res;
		}
		/// <summary>
		...
		{
			float res = pmData.basePowerLevel;
			foreach (IRoomPowerModifier unit in subs) { if (unit.Enabled) res += unit.GlobalBonus(); }
			res = Clamp01(res);
			return res;
		}
		...
		internal List<IRoomPowerModifier> subs = new();
		/// <summary>
