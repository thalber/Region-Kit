...
namespace RegionKit.Machinery
{
	...
	public static class MachineryStatic
	{
		...
		public static void Enable()
		{
			...
			foreach (Hook h in MachineryHooks) h.Apply();
		}
		...
		{
			foreach (Hook h in MachineryHooks) h.Undo();
		}
		...
		{
			orig(self, obj);
			if (ManagersByRoom.TryGetValue(self.GetHashCode(), out RoomPowerManager? manager) && obj is RoomPowerManager.IRoomPowerModifier rpm) manager.RegisterPowerDevice(rpm);
		}
		...
		public static readonly Dictionary<int, RoomPowerManager> ManagersByRoom = new();
