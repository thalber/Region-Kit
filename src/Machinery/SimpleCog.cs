...
namespace RegionKit.Machinery
{
	public class SimpleCog : UpdatableAndDeletable, IDrawable
	{
		...
		internal SimpleCogData cData
		{
			get
			{
				SimpleCogData? r = _assignedCD ?? PO?.data as SimpleCogData;
				if (r == null) { _bcd ??= new SimpleCogData(null); return _bcd; }
				return r;
			}
		}
		...
		internal float cAngVel
		{
			get
			{
				float res = Lerp(0f, cData.baseAngVel, room.GetGlobalPower());
				Func<float, float> targetFunc = cData.opmode switch
				{
					OperationMode.Cosinal => Cos,
					_ => Sin,
				};
