...
namespace RegionKit.Machinery
{
	public class SimplePiston : UpdatableAndDeletable, IDrawable
	{
		...
		internal PistonData mData
		{
			get
			{
				PistonData? r = _assignedMData ?? PO?.data as PistonData;
				if (r == null) { _bpd ??= new PistonData(null); return _bpd; }
				return r;
			}
		}
		...
		internal float Shift
		{
			get
			{
				float res = mData.amplitude;
				Func<double, double> chosenFunc = mData.opmode switch
				{
					OperationMode.Cosinal => Math.Cos,
					_ => Math.Sin,
				};
		...
		{
			_mc ??= room.roomSettings.placedObjects.FirstOrDefault(
		...
		{
			Vector2 pos = Vector2.Lerp(oldPos, currentPos, timeStacker);
		...
		{
			foreach (FSprite? fs in sLeaser.sprites) fs.RemoveFromContainer();
