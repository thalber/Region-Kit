...
namespace RegionKit.Machinery
{
	public class PistonArray : UpdatableAndDeletable
	{
		...
		public override void Update(bool eu)
		{
			...
			{
				Tuple<PistonData, SimplePiston> pair = pistons[i];
				PistonData ndt = pdByIndex(i);
		...
		internal void CleanUpPistons()
		{
			if (pistons != null)
			{
				foreach (Tuple<PistonData, SimplePiston> pair in pistons) { pair.Item2.Destroy(); }
			}
			pistons = null;
		}
		internal void GeneratePistons()
		{
			...
			for (int i = 0; i < pistons.Length; i++)
			{
				PistonData pdata = pdByIndex(i);
				SimplePiston piston = new(room, null, pdata);
