...
namespace RegionKit.Objects
{
	public class ColouredLightSource : UpdatableAndDeletable
	{
		...
		public ColouredLightSource(PlacedObject pObj, Room room)
		{
			...
			{
				affectedByPaletteDarkness = Data?.GetValue<float>("paletteDarkness") ?? 0.5f
			};
		...
		{
			float aux = Random.value * Mathf.PI * 2f;
			float vectorX = Mathf.Cos(aux);
			float vectorY = Mathf.Sin(aux);

			float noiseValue = Mathf.Clamp01(Mathf.PerlinNoise(vectorX * Time.time, vectorY * Time.time));
			return noiseValue;
		}

		public static void RegisterAsFullyManagedObject()
		{
			PlacedObjectsManager.RegisterFullyManagedObjectType(Fields, typeof(ColouredLightSource));
		}
	}
}