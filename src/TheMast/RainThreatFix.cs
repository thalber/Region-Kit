...
namespace RegionKit.TheMast
{
	internal static class RainThreatFix
	{
		...
		private static void WorldLoader_LoadAbstractRoom(On.WorldLoader.orig_LoadAbstractRoom orig, World world, string roomName, AbstractRoom room, RainWorldGame.SetupValues setupValues)
		{
			...
			{
				RoomSettings settings = new(roomName, world.region, false, false, world.game.StoryCharacter);
		...
		public static float GetRainThreat(AbstractRoom room)
		{
			return GetRainThreat(room.index, out _);
		}

		public static float GetRainThreat(AbstractRoom room, out bool hasThreat)
		{
			return GetRainThreat(room.index, out hasThreat);
		}

		public static float GetRainThreat(int room)
		{
			return GetRainThreat(room, out _);
		}
