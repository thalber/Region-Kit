	...
	internal static class PearlChains
	{
		...
		public static void Room_Loaded(On.Room.orig_Loaded orig, Room self)
		{
			...
			if (firstTimeRealized)
			{
				for (int i = 0; i < self.roomSettings.placedObjects.Count; i++)
				{
					...
					AbstractPearlChain apo = new(self.world, EnumExt_PearlChains.PearlChain, null, self.GetWorldCoordinate(pObj.pos), self.game.GetNewID(), self.abstractRoom.index, i, pObj.data as PlacedPearlChainData, (pObj.data as PlacedPearlChainData)?.length ?? 0)
					{
						isConsumed = false
					};
		...
		public static void ObjectsPage_CreateObjRep(On.DevInterface.ObjectsPage.orig_CreateObjRep orig, ObjectsPage self, PlacedObject.Type tp, PlacedObject pObj)
		{
			if (tp == EnumExt_PearlChains.PlacedPearlChain)
			{
				...
				ConsumableRepresentation rep = new(self.owner, tp.ToString() + "_Rep", self, pObj, tp.ToString());
		...
		public static void ConsumableRepresentation_ctor(On.DevInterface.ConsumableRepresentation.orig_ctor orig, ConsumableRepresentation self, DevUI owner, string IDstring, DevUINode parentNode, PlacedObject pObj, string name)
		{
			...
			if (pObj.type == EnumExt_PearlChains.PlacedPearlChain)
			{
				...
				ConsumableRepresentation.ConsumableControlPanel cp = (ConsumableRepresentation.ConsumableControlPanel)_ConsumableRepresentation_controlPanel.GetValue(self);
		...
		public class PearlChain : PlayerCarryableItem, IDrawable
		{
						...
						if (grabbedBy[0].grabber is Player theif)
						{
							...
							if (!_lastGrabbed)
							{
								...
								for (int i = 0; i < creatures.Count; i++)
								{
									...
									if (creature.realizedCreature is not Scavenger scav) continue;
			...
			public void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
			{
				...
				for (int i = 0; i < pearlCount; i++)
				{
					...
					if (i > 0)
					{
						...
						Vector2 right = new(delta.y, -delta.x);
						right.Normalize();
			...
			private static class PearlVars
			{
				...
				{
					return _colors[(color == -1) ? 0 : color * 2];
				}
