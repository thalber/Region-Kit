...
#pragma warning restore IDE0051 // Remove unused private members
        ...
        /// <param name="name">name of the object. do not allow duplicates</param>
        /// <param name="singleinstance">whether only one instance is allowed per room</param>
        ...
        {
            //var uadc = UAD_CTOR<UAD>();
            //if (uadc is null) throw new ArgumentException($"{typeof(UAD).ToString()}: UAD type does not have a suitable constructor ((room, placedobject) / (placedobject, room) / (room) / (placedobject))");
        ...
        private static ConstructorInfo UAD_CTOR<UAD>() where UAD : UpdatableAndDeletable
        {
            var po_t = typeof(PlacedObject);
            var rm_t = typeof(Room);
            var uad_t = typeof(UAD);
            foreach (Type[] callvar in new Type[][] {
                new[] { po_t },
                new[] { rm_t },
                new[] { po_t, rm_t },
                new[] { rm_t, po_t } })
            {
                var ctorinfo = uad_t.GetConstructor(allContextsCtor,
                    null,
                    CallingConventions.Standard,
                    callvar,
                    null);
                if (ctorinfo is not null) return ctorinfo;
            }
            return null;
        }
        #region MANAGED
